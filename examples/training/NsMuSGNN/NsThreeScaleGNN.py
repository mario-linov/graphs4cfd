...
dataset = gcfd.datasets.NsCircle(format='uvp', path=path, training_info={"n_in":1, "n_out":train_config['num_steps'][-1], "step":1, "T":100}, transform=transform, preload=True) # Replace with path to nsCircle.h5
...
model = gcfd.nn.NsThreeScaleGNN(arch=arch)
